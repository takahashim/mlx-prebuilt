...
# in the box.
# Found by the licence file rather than by the directory it sits in. An
# out-of-tree source leaves an object-only mirror of its name under
# CMakeFiles, and a search by name can land on that one, which holds the
# objects and not the licence.
cp "$work/mlx-c/LICENSE" "$OUT/LICENSE.mlx-c"
mlx_license="$(find "$work/build" -type f -path '*/mlx-src/LICENSE' -print -quit)"
if [ -z "$mlx_license" ]; then
  echo "!! MLX's LICENSE was not found; the archive may not be redistributed without it" >&2
  exit 1
fi
cp "$mlx_license" "$OUT/LICENSE.mlx"
mlx_src="${mlx_license%/LICENSE}"
...
# is the MIT notice.
gguf_license="$(find "$work/build" -type f -path '*/gguflib-src/LICENSE' -print -quit)"
if [ -z "$gguf_license" ]; then
  echo "!! gguflib's LICENSE was not found; the archive may not be redistributed without it" >&2
  exit 1
fi
cp "$gguf_license" "$OUT/LICENSE.gguflib"
